...
# ใส่ User ID ของคุณ (ที่ขึ้นต้นด้วย U...)
LINE_USER_ID = "U822497c4e7ad05ffe52d957ec2cb4a31"

# [imgbb] (ฝากรูปฟรี)
# สมัครที่ https://api.imgbb.com/ แล้วเอา API Key มาใส่
...
# [LOGIC PARAMETERS]
FALL_CONFIRM_FRAMES = 5      # ล้มนาน 5 เฟรมถึงเริ่มนับ
HEIGHT_DROP_THRES = 0.65     # ความสูงเหลือต่ำกว่า 65% = ล้ม
ASPECT_RATIO_THRES = 1.1     # กว้างมากกว่าสูง = ล้ม
ANGLE_THRES = 45             # มุมลำตัวเอียง
API_COOLDOWN = 30            # ส่ง LINE ซ้ำทุกๆ 30 วินาที (ถ้ายังล้มอยู่)

# ==============================================================================
# 2. ระบบหลัก
# ==============================================================================

class FallDetectorReal:
    def __init__(self):
        print("[System] Initializing Real-Time System...")
        ...
        # ตัวแปรระบบ
        self.history = defaultdict(lambda: deque(maxlen=150)) # จำประวัตินานขึ้น
        self.fall_counters = defaultdict(int) 
        self.is_falling_state = defaultdict(bool) # จำสถานะว่ากำลังล้มอยู่ไหม
        self.last_api_sent_time = {} # จำเวลาที่ส่ง LINE ล่าสุดของแต่ละคน
        ...
        while True:
            if self.msg_queue:
                ...
                if img_path and os.path.exists(img_path):
                    print(f"[System] Sending Alert...")
                    img_link = self.upload_imgbb(img_path)
                
                # 2. Notify
                self.send_line_oa(msg_text, img_link)
                print(f"[System] LINE Sent: {msg_text.splitlines()[0]}")
    ...
    def trigger_api_alert(self, person_id, frame, score):
        now = time.time()
        # เช็ค Cooldown เฉพาะการส่ง API (หน้าจอยังแดงตลอด)
        ...
        while True:
            ...
            if results[0].boxes.id is not None:
                ...
                for i, track_id in enumerate(track_ids):
                    ...
                    # ถ้าสถานะคือ "กำลังล้ม" -> หยุดจำความสูง (Freeze) 
                    # เพื่อให้ค่าเฉลี่ย (Ref) ยังคงเป็นค่าตอนยืนอยู่ตลอด
                    ...
                    if kp.shape[0] > 12: 
                        ...
                        if s_mid and h_mid:
                            if self.calculate_angle(s_mid, h_mid) > ANGLE_THRES: risk_angle = True
                    ...
                    if self.fall_counters[track_id] >= FALL_CONFIRM_FRAMES:
                        self.is_falling_state[track_id] = True # ล็อกสถานะว่าล้ม
                        status, color = "FALL DETECTED!", (0, 0, 255)
                        
                        # ส่ง API (มี Cooldown 30 วิ กันรัว)
                        self.trigger_api_alert(track_id, frame, drop_ratio)
                        
                    else:
                        self.is_falling_state[track_id] = False # ปลดล็อก
                        status, color = "Normal", (0, 255, 0)
                        if self.fall_counters[track_id] > 2: status, color = "Warning...", (0, 165, 255)
